(() => {
  ...
  zh: {
    ...
    'set.appearance': '外观', 'set.plainUi': '素色', 'set.fontSize': '聊天字号', 'set.responseDisplay': '回答显示', 'set.expandThinking': '默认展开思考', 'set.expandTools': '默认展开工具调用', 'set.chat': '聊天', 'set.defaultKbEnabled': '默认启用知识库', 'set.lang': '语言', 'set.model': '模型', 'set.kbVisionHint': '知识库图片理解默认复用当前默认模型（也可配置独立视觉模型）并会发送图片；请确认所用模型支持多模态输入。纯文本模型仍可导入文本，但图片只保留图号、图注和来源。', 'set.addModel': '添加模型', 'set.features': '功能', 'set.importMykey': '导入模型配置', 'set.exportMykey': '导出模型配置', 'set.importMemory': '导入记忆与会话记录', 'set.gaSource': '接入外部 GenericAgent 源码', 'set.gaSourceClear': '改用内置版本', 'set.gaSourceCurrent': '当前接入', 'set.serviceManager': '后台服务管理',
    ...
    'pq.title': '新用户引导', 'pq.sub': '按三步完成首次配置', 'pq.toggle': '展开 / 收起',
    ...
    'pq.embeddingAction': '嵌入模型', 'pq.embeddingActionDesc': '用于知识库语义检索；可继承阿里云百炼 API Key',
    'pq.mineruTitle': '配置 MinerU',
    'pq.mineruAction': 'MinerU', 'pq.mineruActionDesc': '用于解析 PDF/Office；填写 API Key 后即可导入',
    'pq.configured': '已配置', 'pq.toConfigure': '待配置',
    'pq.embeddingModalTitle': '配置嵌入模型', 'pq.embeddingGuide1': '嵌入模型把文本转换为向量，知识库才能进行语义检索。', 'pq.embeddingGuide2': '如果已配置阿里云百炼模型，API Key 可以留空并自动复用。当前默认值已填入阿里云配置。', 'pq.embeddingGuide3': '默认使用 text-embedding-v4 / 1024 维，通常无需修改。', 'pq.embeddingGuideTip': '保存后即可在导入知识库时使用。',
    'pq.mineruModalTitle': '配置 MinerU', 'pq.mineruGuide1': 'MinerU 负责把 PDF/Office 文档解析成可检索内容。', 'pq.mineruGuide2': '注册 MinerU 账号并获取 API Key。', 'pq.mineruGuide3': '填写 Key 后保存，再从知识库页面导入文档。', 'pq.mineruRegister': '注册并获取 MinerU API Key', 'pq.mineruGuideTip': '解析服务由 MinerU 提供，源文档不会被修改。',
    ...
    'page.kb.title': '知识库',
    'kb.import': '导入', 'kb.build': '重建索引', 'kb.importResult': '导入结果', 'kb.reindexResult': '重建结果',
    ...
    'err.kbLoad': '加载知识库失败', 'err.kbImport': '导入知识库失败', 'err.kbFolderPicker': '当前环境不支持文件夹选择，请使用桌面版打开', 'err.kbBuild': '构建索引失败', 'err.kbDelete': '删除知识库失败', 'err.kbAsk': '知识库问答失败',
  ...
  en: {
    ...
    'set.appearance': 'Appearance', 'set.plainUi': 'Plain', 'set.fontSize': 'Chat font size', 'set.responseDisplay': 'Response display', 'set.expandThinking': 'Expand thinking by default', 'set.expandTools': 'Expand tool calls by default', 'set.chat': 'Chat', 'set.defaultKbEnabled': 'Enable knowledge bases by default', 'set.lang': 'Language', 'set.model': 'Model', 'set.kbVisionHint': 'Knowledge-base image understanding reuses the current default model by default (or a dedicated vision model if configured) and sends it images; make sure the selected model accepts multimodal input. Text-only models can still import text, while images keep only their number, caption, and source.', 'set.addModel': 'Add model', 'set.features': 'Features', 'set.importMykey': 'Import model config', 'set.exportMykey': 'Export model config', 'set.importMemory': 'Import memory and sessions', 'set.gaSource': 'Connect external GenericAgent source', 'set.gaSourceClear': 'Use bundled version', 'set.gaSourceCurrent': 'Connected to', 'set.serviceManager': 'Service manager',
    ...
    'pq.title': 'New user guide', 'pq.sub': 'Complete the setup in three steps', 'pq.toggle': 'Expand / collapse',
    ...
    'kb.import': 'Import', 'kb.build': 'Reindex', 'kb.importResult': 'Import result', 'kb.reindexResult': 'Reindex result',
    ...
    'err.kbLoad': 'Failed to load knowledge base', 'err.kbImport': 'Knowledge-base import failed', 'err.kbFolderPicker': 'Folder selection is unavailable here; open the desktop app instead', 'err.kbBuild': 'Index build failed', 'err.kbDelete': 'Knowledge-base deletion failed', 'err.kbAsk': 'Knowledge-base question failed',
