...
install_dependencies() {
  ...
  if [[ -n "$WHEEL_DIR" ]]; then
    ...
    "$py" -m pip install --no-index --find-links "$WHEEL_DIR" \
      "requests>=2.28" "beautifulsoup4>=4.12" "bottle>=0.12" "simple-websocket-server>=0.4" "aiohttp>=3.9" psutil "docling-slim[format-markdown,feat-chunking]==2.115.0" "zvec>=0.6,<0.7" "pypdf>=5.0" $EXTRA_PACKAGES \
