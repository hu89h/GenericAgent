...
| 3 原生图片输入 | 2仍不足时才用 | 用 `file_read` 把局部窗口截图交给当前模型做语义理解、确认界面状态 | 当前模型必须已验证支持图片；不可信模型给出的坐标 |
...
截图前必须先枚举并确认目标窗口；禁止为了省事截取整个桌面。优先截目标窗口或完成任务所需的最小局部区域，避免把其他窗口、通知和隐私信息带入模型输入。
需要模型判断视觉内容时，可用 `code_run + ljqCtrl` 把目标窗口截图保存到固定临时文件，再调用 `file_read`。`file_read` 会将原图加入下一轮模型输入；若当前模型未验证图片能力，应停止并提示用户切换或重新验证模型，不能另调视觉模型、OCR 或知识库 VLM 代答。
