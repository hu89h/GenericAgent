...
class ZvecIndex:
    ...
    def embed_sparse(self, texts: list[str], *, text_type: str = "document"):
        return embeddings.embed_sparse_texts(texts, text_type=text_type)

    def insert_records(self, kb: dict, collection, records, logfn=None):
        zvec = self.require()
        batch: list[dict] = []
        counts = {"n_chunks": 0, "text_chunks": 0, "image_chunks": 0}
        ...
        def flush() -> None:
            ...
            vectors = self.embed_dense(texts)
            sparse_vectors = self.embed_sparse(texts, text_type="document")
    ...
    def build(self, kb: dict, records, sources: dict, logfn=None):
